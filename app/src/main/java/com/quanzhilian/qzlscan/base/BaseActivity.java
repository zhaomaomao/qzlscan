...
public class BaseActivity extends Activity {
    ...
    /* 是否登录，仅判断本地登录状态*/
    protected void isLogin(BaseActivity currentActivity) {
    ...
    /* 是否登录，仅判断本地登录状态*/
    protected boolean isLogined(BaseActivity currentActivity) {
        String sessonId = GlobleCache.getInst().getCacheSessionId();
    ...
    protected void getRepositoryList(final Activity activity, final Handler mHandler,final boolean showCancel) {
        ...
        if (repositoryList != null) {
            if(repositoryList.size() != 0){
                selectRepository(true,mHandler, activity, repositoryList);
            }else{
