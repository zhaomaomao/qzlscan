...
public class OutRepositoryItemDetailActivity extends Activity implements View.OnClickListener {
    //头部公共区域
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_out_repository_item_detail);
...
//        }
    ...
    private void bindView() {
        if (!StringUtils.isEmpty(repositoryCuttingItemId)) {
            scanTon = 0d;
            repositoryBillItemModel = DBManager.getInstance(this).queryInBillItem(repositoryCuttingItemId, EnumQueryType.out_repository_bill.getVal() + "");
            String unitName = "";
            ...
            rpositoryBillItemDetailModelList = DBManager.getInstance(OutRepositoryItemDetailActivity.this).queryInBillItemDetailList(repositoryCuttingItemId, EnumQueryType.out_repository_bill.getVal() + "","1");
            ...
            if (detailAdapter == null) {
                detailAdapter = new RepositoryDetailAdapter(OutRepositoryItemDetailActivity.this, rpositoryBillItemDetailModelList);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_titilebar_button:
                //清除已扫
                new android.support.v7.app.AlertDialog.Builder(OutRepositoryItemDetailActivity.this)
                        .setTitle("提示")
                        ...
                        .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                DBManager.getInstance(OutRepositoryItemDetailActivity.this).delBillDetailByItemId(repositoryCuttingId, repositoryCuttingItemId, EnumQueryType.out_repository_bill.getVal() + "");
                                //initViewData();
                                bindView();
                            }
                        })
            ...
            case R.id.tv_second_oper:
                if (model.isFull == 1) {
                    //是整件出
                    DBManager.getInstance(this).updateBillDetailIsFullByRepositoryProductId(repositoryCuttingItemId, model.repositoryProductId.toString(), "0", EnumQueryType.out_repository_bill.getVal() + "");
                    bindView();
                } else {
                    //是拆零出
                    DBManager.getInstance(this).updateBillDetailIsFullByRepositoryProductId(repositoryCuttingItemId, model.repositoryProductId.toString(), "1", EnumQueryType.out_repository_bill.getVal() + "");
                    bindView();
                }
                mCameraDialog.dismiss();
                break;
                ...
                if (model != null) {
                    new android.support.v7.app.AlertDialog.Builder(OutRepositoryItemDetailActivity.this)
                            ...
                            .setPositiveButton("确认", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    DBManager.getInstance(OutRepositoryItemDetailActivity.this).delBillDetailByRepositoryProductId(repositoryCuttingItemId, model.repositoryProductId.toString(), EnumQueryType.out_repository_bill.getVal() + "");
                                    bindView();
                                }
                            })
                            .create().show();
    ...
    private void setDialog(RpositoryBillItemDetailModel model) {
        ...
        if (model.isFull == 1) {
            tv_second_oper.setText("拆零出");
        } else {
            tv_second_oper.setText("整件出");
        }
