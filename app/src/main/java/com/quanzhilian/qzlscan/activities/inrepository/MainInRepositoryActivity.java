...
public class MainInRepositoryActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initTitle() {
        ...
        common_header_title.setText(getResources().getString(R.string.check_in_bill));
    ...
    private void initView() {
        ...
        progressDialog.setMessage("入库单下载中...");
        ...
        ll_scan_in_bill = (LinearLayout) findViewById(R.id.ll_scan_in_bill);
        ll_download_bill = (LinearLayout) findViewById(R.id.ll_download_bill);

        //btn_change_repository.setOnClickListener(this);
        //ll_scan_in_bill.setOnClickListener(this);
        ll_download_bill.setOnClickListener(this);
        showRepository();

        ll_scan_in_bill.setOnTouchListener(new View.OnTouchListener() {
            @Override
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.ll_download_bill:
                ...
                if (isLogined(this)) {
                    //获取查询时间
                    CommonRequest.getServiceQueryTime(MainInRepositoryActivity.this, mHandler);
                } else {
                    return;
                }
                break;
            ...
            switch (msg.what) {
                ...
                case MessageWhat.MessageType.GET_QUERY_TIEM_SUCCESS:
                    if (msg != null && msg.obj != null) {
                        ...
                        lastSearchDate = DBManager.getInstance(MainInRepositoryActivity.this).getLastSearchDate(EnumQueryType.in_repository_bill, nowLastSearchDate, GlobleCache.getInst().getCacheRepositoryId(), GlobleCache.getInst().getScmId());
                        String scmId = GlobleCache.getInst().getScmId();
                        getAllInBillList(scmId);
                    }
                    break;
                case MessageWhat.MessageType.BILL_APPROVEED:
                    final String billId = msg.obj.toString();
                    AlertDialog alertDialog = new AlertDialog.Builder(MainInRepositoryActivity.this)
                            ...
                            .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    dialog.dismiss();
                                    DBManager.getInstance(MainInRepositoryActivity.this).delBill(billId,EnumQueryType.in_repository_bill.getVal()+"");
                                }
                            }).create();
    ...
    private void getAllInBillList(final String scmId) {
        if (NetWorkUtils.isNetWork(MainInRepositoryActivity.this)) {
            ...
            try {
                ...
                String requestUrl = UrlUtils.getFullUrl(UrlConstant.url_get_bill_list);
                httpClientUtil.postRequest(requestUrl, paras);
                progressDialog.show();//启动加载进度动画
            } catch (Exception ex) {
                progressDialog.dismiss(); //关闭加载进度动画
                Toast.makeText(MainInRepositoryActivity.this, R.string.server_connect_error, Toast.LENGTH_SHORT).show();
            } finally {
                                                ...
                                                if (result == null) {
                                                    ...
                                                    try {
                                                        ...
                                                        if (jsonRequestResult.getCode() == 1 && jsonRequestResult.getObj() != null) {
                                                            repositoryBillModelList = jsonRequestResult.getResultBeanList(SimpleRepositoryBillModel.class);
...
//                                                                }
                                                                DBManager.getInstance(MainInRepositoryActivity.this).addRpositoryBill(repositoryBillModelList, EnumQueryType.in_repository_bill.getVal());
                                                            }
                                                            progressDialog.dismiss();
                                                            //数据下载成功后打开列表页
                                                            isScanActive = false;
                                                            ...
                                                            isOnlineViaesult(MainInRepositoryActivity.this, jsonRequestResult);
    ...
    private void getInBillDetail(final String rpositoryBillNo) {
        //先从本地数据库查询
        RpositoryBillModel rpositoryBillModel = DBManager.getInstance(MainInRepositoryActivity.this).getBill(rpositoryBillNo, EnumQueryType.in_repository_bill.getVal() + "");
        if (rpositoryBillModel != null && rpositoryBillModel.repositoryBillId > 0) {
            ...
            if (NetWorkUtils.isNetWork(MainInRepositoryActivity.this)) {
                ...
                try {
                    /*请求服务端*/
                    Map<String, String> paras = new HashMap<String, String>();
                    paras.put("repositoryBillNo", rpositoryBillNo);
                    paras.put("repositoryId", GlobleCache.getInst().getCacheRepositoryId());
                                                        ...
                                                        try {
                                                            ...
                                                            if (jsonRequestResult.getCode() == 1) {
                                                                if (jsonRequestResult.getObj() != null) {
                                                                    SimpleRepositoryBillModel rpositoryBillModel = jsonRequestResult.getResultObjBean(SimpleRepositoryBillModel.class, "repositoryBill");
                                                                    if (rpositoryBillModel != null) {
                                                                        if (rpositoryBillModel.getState() == 2) {
                                                                            //入库单已审核，无需再扫：弹出提示
                                                                            Message message = new Message();
                                                                            message.what = MessageWhat.MessageType.BILL_APPROVEED;
                                                                            message.obj = rpositoryBillModel.getRepositoryBillId();
                                                                            mHandler.sendMessage(message);
                                                                            //mHandler.sendEmptyMessage(MessageWhat.MessageType.BILL_APPROVEED);
                                                                        } else {
                                                                            repositoryBillModelList = new ArrayList<SimpleRepositoryBillModel>();
                                                                            repositoryBillModelList.add(rpositoryBillModel);
                                                                            DBManager.getInstance(MainInRepositoryActivity.this).addRpositoryBill(repositoryBillModelList, EnumQueryType.in_repository_bill.getVal());
                                                                            //数据下载成功后打开详情页
                                                                            isScanActive = false;
                                                                            Bundle bundle = new Bundle();
                                                                            bundle.putString("repositoryBillId", rpositoryBillModel.getRepositoryBillId().toString());
                                                                            jumpActivity(InRepositoryDetailActivity.class, bundle);
                                                                        }
                                                                    } else {
                                                                        forToast("单据不存在！");
    ...
    private String getRepositoryBillIds() {
        String billIds = "";
        String scmId = GlobleCache.getInst().getScmId();
        List<RpositoryBillModel> rpositoryBillModels = DBManager.getInstance(this).queryBillList(scmId, GlobleCache.getInst().getCacheRepositoryId(), "1,2,3", EnumQueryType.in_repository_bill.getVal() + "");
